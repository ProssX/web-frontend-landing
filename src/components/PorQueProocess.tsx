import { useEffect, useRef } from 'react'

type Rating = 'pos' | 'mid' | 'neg'

const ROWS: {
  label: string
  trad: Rating; tradText: string
  manual: Rating; manualText: string
  proocess: Rating; proocText: string
}[] = [
  { label: 'Velocidad',        trad: 'neg', tradText: 'Semanas',      manual: 'mid', manualText: 'Días',         proocess: 'pos', proocText: 'Horas'       },
  { label: 'Costo',            trad: 'neg', tradText: 'Alto',         manual: 'mid', manualText: 'Moderado',     proocess: 'pos', proocText: 'Eficiente'   },
  { label: 'Escalabilidad',    trad: 'neg', tradText: 'Baja',         manual: 'mid', manualText: 'Media',        proocess: 'pos', proocText: 'Alta'        },
  { label: 'Consistencia',     trad: 'neg', tradText: 'Variable',     manual: 'neg', manualText: 'Variable',     proocess: 'pos', proocText: 'Garantizada' },
  { label: 'Implementación',   trad: 'neg', tradText: 'Compleja',     manual: 'mid', manualText: 'Manual',       proocess: 'pos', proocText: 'Guiada'      },
  { label: 'Soporte continuo', trad: 'neg', tradText: 'Por proyecto', manual: 'neg', manualText: 'Sin soporte',  proocess: 'pos', proocText: 'Incluido'    },
]

const BADGE: Record<Rating, { bg: string; color: string; icon: string }> = {
  pos: { bg: 'rgba(52,211,153,0.12)',  color: '#6EE7B7', icon: '✓' },
  mid: { bg: 'rgba(251,191,36,0.12)',  color: '#FCD34D', icon: '~' },
  neg: { bg: 'rgba(249,80,104,0.10)',  color: '#FDA3AF', icon: '✗' },
}

function Badge({ type, children }: { type: Rating; children: string }) {
  const { bg, color, icon } = BADGE[type]
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: '0.4rem',
      background: bg, color,
      fontSize: '0.8125rem', fontWeight: 500,
      borderRadius: '9999px',
      padding: '0.25rem 0.75rem',
      lineHeight: 1.4,
    }}>
      <span style={{ fontWeight: 700, fontSize: '0.75rem' }}>{icon}</span>
      {children}
    </span>
  )
}

export default function PorQueProocess() {
  const sectionRef = useRef<HTMLElement>(null)

  useEffect(() => {
    const section = sectionRef.current
    if (!section) return
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          section.classList.add('pq-in-view')
          observer.disconnect()
        }
      },
      { threshold: 0.12 }
    )
    observer.observe(section)
    return () => observer.disconnect()
  }, [])

  return (
    <>
      <style>{`
        #porque-proocess {
          --pq-coral-col-bg: rgba(249,80,104,0.055);
          --pq-coral-border: rgba(249,80,104,0.22);
          --pq-row-sep: rgba(247,245,242,0.06);
          --pq-text-muted: rgba(247,245,242,0.42);
        }
        #porque-proocess .pq-header,
        #porque-proocess .pq-row {
          opacity: 0;
          transform: translateY(14px);
          transition: opacity 0.55s ease, transform 0.55s ease;
        }
        #porque-proocess.pq-in-view .pq-header { opacity: 1; transform: none; transition-delay: 0ms; }
        #porque-proocess.pq-in-view .pq-row:nth-child(1) { opacity: 1; transform: none; transition-delay: 80ms;  }
        #porque-proocess.pq-in-view .pq-row:nth-child(2) { opacity: 1; transform: none; transition-delay: 150ms; }
        #porque-proocess.pq-in-view .pq-row:nth-child(3) { opacity: 1; transform: none; transition-delay: 220ms; }
        #porque-proocess.pq-in-view .pq-row:nth-child(4) { opacity: 1; transform: none; transition-delay: 290ms; }
        #porque-proocess.pq-in-view .pq-row:nth-child(5) { opacity: 1; transform: none; transition-delay: 360ms; }
        #porque-proocess.pq-in-view .pq-row:nth-child(6) { opacity: 1; transform: none; transition-delay: 430ms; }
        @media (max-width: 680px) {
          #porque-proocess .pq-table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; }
          #porque-proocess table { min-width: 560px; }
        }
      `}</style>

      <section
        ref={sectionRef}
        id="porque-proocess"
        style={{
          position: 'relative',
          zIndex: 4,
          background: 'var(--color-bg-dark)',
          color: 'var(--color-text-dark)',
          paddingBlock: 'clamp(5rem, 10vw, 8rem)',
          overflow: 'hidden',
        }}
      >
        {/* Decorative gradient */}
        <div aria-hidden="true" style={{
          position: 'absolute', inset: 0, pointerEvents: 'none',
          background: `
            radial-gradient(ellipse 50% 45% at 90% 20%, rgba(249,80,104,0.05) 0%, transparent 65%),
            radial-gradient(ellipse 40% 35% at 10% 80%, rgba(199,231,242,0.03) 0%, transparent 60%)
          `,
        }} />

        <div className="container" style={{ position: 'relative' }}>

          {/* Section header */}
          <div style={{ marginBottom: 'clamp(2.5rem, 5vw, 4rem)', maxWidth: 600 }}>
            <span style={{
              display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
              fontSize: '0.6875rem', fontWeight: 700,
              letterSpacing: '0.12em', textTransform: 'uppercase',
              background: 'rgba(249,80,104,0.10)',
              color: 'var(--color-primary)',
              border: '1px solid rgba(249,80,104,0.20)',
              borderRadius: '9999px',
              padding: '0.3rem 0.9rem',
              marginBottom: '1.25rem',
            }}>
              <span style={{
                width: 5, height: 5, borderRadius: '50%',
                background: 'var(--color-primary)', flexShrink: 0,
              }} />
              Por qué Proocess
            </span>

            <h2 style={{
              fontFamily: 'Syne, sans-serif',
              fontSize: 'clamp(1.875rem, 3.5vw, 2.875rem)',
              fontWeight: 700,
              color: 'var(--color-text-dark)',
              lineHeight: 1.1,
              letterSpacing: '-0.03em',
              margin: '0 0 1rem',
            }}>
              La diferencia que se nota<br />desde el primer día.
            </h2>

            <p style={{
              fontSize: '1rem', lineHeight: 1.65,
              color: 'rgba(247,245,242,0.45)',
              margin: 0, maxWidth: '52ch',
            }}>
              Sin configuraciones eternas ni consultores costosos. Proocess hace
              lo que los otros métodos prometen pero raramente cumplen.
            </p>
          </div>

          {/* Table */}
          <div className="pq-table-wrap">
            <table style={{
              width: '100%',
              borderCollapse: 'separate',
              borderSpacing: 0,
              tableLayout: 'fixed',
            }}>
              {/* Column groups for Proocess highlight */}
              <colgroup>
                <col style={{ width: '30%' }} />
                <col />
                <col />
                <col style={{
                  background: 'var(--pq-coral-col-bg)',
                  borderLeft: '1px solid var(--pq-coral-border)',
                  borderRight: '1px solid var(--pq-coral-border)',
                }} />
              </colgroup>

              <thead>
                <tr className="pq-header">
                  <th style={{ padding: '0 0 1.5rem', textAlign: 'left' }} />

                  {/* Consultoría */}
                  <th style={{
                    padding: '0 1rem 1.5rem',
                    textAlign: 'center',
                    fontSize: '0.875rem',
                    fontWeight: 500,
                    color: 'rgba(247,245,242,0.38)',
                    letterSpacing: '-0.01em',
                  }}>
                    Consultoría<br />
                    <span style={{ fontSize: '0.75rem', fontWeight: 400 }}>tradicional</span>
                  </th>

                  {/* Manual */}
                  <th style={{
                    padding: '0 1rem 1.5rem',
                    textAlign: 'center',
                    fontSize: '0.875rem',
                    fontWeight: 500,
                    color: 'rgba(247,245,242,0.38)',
                    letterSpacing: '-0.01em',
                  }}>
                    Manual<br />
                    <span style={{ fontSize: '0.75rem', fontWeight: 400 }}>/&nbsp;interno</span>
                  </th>

                  {/* Proocess */}
                  <th style={{
                    padding: '0 1rem 1.5rem',
                    textAlign: 'center',
                    background: 'var(--pq-coral-col-bg)',
                    borderTop: '2px solid var(--color-primary)',
                    borderLeft: '1px solid var(--pq-coral-border)',
                    borderRight: '1px solid var(--pq-coral-border)',
                    borderRadius: '0.5rem 0.5rem 0 0',
                  }}>
                    <span style={{
                      display: 'block',
                      fontFamily: 'Syne, sans-serif',
                      fontSize: '0.9375rem',
                      fontWeight: 700,
                      color: 'var(--color-primary)',
                      letterSpacing: '-0.01em',
                      paddingTop: '0.875rem',
                    }}>
                      Proocess
                    </span>
                    <span style={{
                      display: 'inline-flex',
                      marginTop: '0.375rem',
                      fontSize: '0.625rem', fontWeight: 700,
                      letterSpacing: '0.1em', textTransform: 'uppercase',
                      background: 'rgba(249,80,104,0.15)',
                      color: 'var(--color-primary)',
                      border: '1px solid rgba(249,80,104,0.25)',
                      borderRadius: '9999px',
                      padding: '0.2rem 0.6rem',
                    }}>
                      Recomendado
                    </span>
                  </th>
                </tr>
              </thead>

              <tbody>
                {ROWS.map((row, i) => {
                  const isLast = i === ROWS.length - 1
                  const cellBase: React.CSSProperties = {
                    padding: '1.125rem 1rem',
                    borderBottom: isLast ? 'none' : '1px solid var(--pq-row-sep)',
                    textAlign: 'center',
                    verticalAlign: 'middle',
                  }

                  return (
                    <tr key={row.label} className="pq-row">
                      {/* Label */}
                      <td style={{
                        ...cellBase,
                        textAlign: 'left',
                        paddingLeft: 0,
                        fontSize: '0.9375rem',
                        fontWeight: 500,
                        color: 'rgba(247,245,242,0.88)',
                        letterSpacing: '-0.01em',
                      }}>
                        {row.label}
                      </td>

                      {/* Consultoría */}
                      <td style={cellBase}>
                        <Badge type={row.trad}>{row.tradText}</Badge>
                      </td>

                      {/* Manual */}
                      <td style={cellBase}>
                        <Badge type={row.manual}>{row.manualText}</Badge>
                      </td>

                      {/* Proocess */}
                      <td style={{
                        ...cellBase,
                        background: 'var(--pq-coral-col-bg)',
                        borderLeft: '1px solid var(--pq-coral-border)',
                        borderRight: '1px solid var(--pq-coral-border)',
                        borderBottom: isLast
                          ? '2px solid rgba(249,80,104,0.35)'
                          : '1px solid rgba(249,80,104,0.10)',
                        borderRadius: isLast ? '0 0 0.5rem 0.5rem' : 0,
                      }}>
                        <Badge type={row.proocess}>{row.proocText}</Badge>
                      </td>
                    </tr>
                  )
                })}
              </tbody>
            </table>
          </div>
        </div>
      </section>
    </>
  )
}
